...
describe('IncomingEventService', () => {
  ...
  function verifyPersistedEvent(savedEvent: IncomingEvent, currentTime) {
    ...
    expect(savedEvent.createdAt.getTime()).toBeGreaterThanOrEqual(
      currentTime.getTime(),
    );
    expect(savedEvent.updatedAt.getTime()).toBeGreaterThanOrEqual(
      currentTime.getTime(),
    );
    expect(savedEvent.eventTime.getTime()).toBeGreaterThanOrEqual(
      currentTime.getTime(),
    );
